  ...
  return (
    <div className="rounded-xl bg-slate-50 p-4 ring-1 ring-slate-200">
      ...
      <p className="mt-1 text-[11px] text-slate-500">
        Today’s watchlist brief, straight to your inbox — no subscription needed.
      </p>
...
export function SubscribePanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setBusy(true);
    // Single Watchlist feed in this edition — subscribe to it directly.
    const res = await subscribe({ email: email.trim(), days, hour, feeds: ['watchlist'] });
    setBusy(false);
